...
sudo apt-get install ros-melodic-mavros ros-melodic-mavros-extras python3-empy -y
...
sudo -H pip3 install catkin-pkg rospkg
...
# source the workspace so that every bash profile can find it --- not needed as this is done in "microav_mush_stack.sh"
